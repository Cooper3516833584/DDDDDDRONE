...
地面站代码中的 HC-14 波特率保持 `9600`。打开 CH340 串口时必须显式关闭 DTR 和 RTS：
...
飞控无线模块 <-> 地面站 HC-14：透明无线链路，地面站 CH340 为 9600 8N1
```

GroundStationLink 不转发飞控姿态或控制原始帧，也不得把地面站 HC-14 的 `9600`、飞控无线 USART2 的 `115200` 或飞控 USB 的 `500000` 混为同一参数。
