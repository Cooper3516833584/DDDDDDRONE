...
- 互换后的 PING/ACK 尚未执行：机载 `2025_嵌赛v2.py` 正在独占飞控服务 5654，未确认任务安全停止前不得抢占或终止，不能标记为硬件联调通过。
...
5. 两块 HC-14 当前均已回读确认或设置为 `B115200 / C28 / S8`；若再次更换模块，必须先在带 SET/KEY 控制的 USB 转串口上确认参数，现有 UT2 只有 TX/RX，不能通过飞控无线桥可靠修改失配模块的 AT 参数。
