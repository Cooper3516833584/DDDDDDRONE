...
- 115200 下真实 PING 重传 3 次后 ACK 超时；原始双向探针中，地面到机载的无线回调为 0 次，机载到地面只收到 3 个乱码字节且没有完整 `BB 33` 帧；
- 该结果与机载 HC-14 仍保留 9600 本地 UART、而飞控 UT2 固定为 115200 的波特率失配一致，不能标记为硬件联调通过。
...
5. 将机载 HC-14 临时接到带 SET/KEY 控制的 USB 转串口，回读并设置为 `B115200 / C28 / S8` 后再接回 UT2；现有 UT2 只有 TX/RX，不能通过飞控无线桥可靠修改失配模块的 AT 参数。
