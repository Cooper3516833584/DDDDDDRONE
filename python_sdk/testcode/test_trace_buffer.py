...
from fleet_bus.models import (  # noqa: E402
    NodeFlags,
    TraceReportFlags,
    TraceRequestPayload,
    TraceSample,
...
from fleet_bus.trace_buffer import (  # noqa: E402
    PoseTraceBuffer,
    PoseTraceSampler,
    TraceSamplingOptions,
    air_state_to_trace_sample,
)
from fleet_bus.protocol import ProtocolError  # noqa: E402
...
class PoseTraceSamplerTests(unittest.TestCase):
    ...
    def test_air_adapter_maps_all_pose_fields(self):
        ...
        self.assertEqual(
            air_state_to_trace_sample(state),
            TraceSample(1234, -10, 20, -30, 35999, 3, 0),
        )

    def test_air_adapter_maps_pose_valid_flag(self):
        state = SimpleNamespace(
            node_uptime_ms=1,
            x_cm=0,
            y_cm=0,
        ...
        sample = air_state_to_trace_sample(state)
        self.assertEqual(sample.flags, int(TraceSampleFlags.POSE_VALID))

    def test_air_adapter_maps_localization_degraded_flag(self):
        state = SimpleNamespace(
            node_uptime_ms=1,
            x_cm=0,
            y_cm=0,
