"""LCUS 系列 8 路 USB 继电器任务层驱动（板载 CH340 转串口）。

硬件与协议（来自厂家说明与已知规律）:

- 设备: LCUS 型 USB 继电器, 8 路, 板载 CH340 转串口, 固定 9600 8N1;
...
- 本模块位于 ``FlightController/Components`` 下, 与 ``UartScreen``/``LD_Radar`` 等组件驱动同级,
...
- 无硬件互锁: 本驱动走独立 USB 串口, 不经过飞控, 因此不受飞控 ACK/重试/心跳/超时保护,
  也没有任何硬件互锁。不要把它当作飞行安全关键回路; 急停、超时和安全兜底由任务层负责。
...
def list_serial_ports() -> List[Tuple[str, str]]:
    ...
    try:
        ...
        logger.warning("[RELAY] 未安装 pyserial, 无法枚举串口")
        return []
...
class LCUSRelay(object):
    """LCUS 8 路 USB 继电器驱动 (CH340 转串口, 9600 8N1)。
    ...
    def open(self) -> None:
        ...
        with self._lock:
            ...
            logger.info(f"[RELAY] 已打开 {self._port} @ {self._baudrate} 8N1")
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.warning(f"[RELAY] 关闭串口失败: {exc}")
            return
        logger.info(f"[RELAY] 已关闭 {self._port}")
        ...
        with self._lock:
            ...
            for attempt in range(attempts):
                ...
                if attempt + 1 < attempts:
                    logger.warning(f"[RELAY] 第{channel}路 {state_text} 校验失败({actual}), 重发第 {attempt + 2} 次")
                else:
                    logger.error(f"[RELAY] 第{channel}路 {state_text} 校验失败({actual})")
            return False
    ...
    def set_all(self, on: bool, verify: bool = False, retries: int = 1) -> bool:
        ...
        if failed:
            logger.error(f"[RELAY] 全部置 {state_text} 失败通道: {failed}")
            return False
        logger.info(f"[RELAY] 全部 {self.channel_count} 路已置 {state_text}")
        return True
    ...
    def all_on(self, verify: bool = False, retries: int = 1) -> bool:
        ...
        多路同时动作与保持时间均未在实机上验证, 不要用于飞行中的关键动作。
        """
    ...
    def query_status(self, timeout: Optional[float] = None) -> Optional[Dict[int, bool]]:
        ...
        if states is None:
            logger.warning("[RELAY] FF 状态查询失败: 未收到有效返回")
        else:
            logger.info(f"[RELAY] FF 状态查询: {format_states(states)}")
        return states
    ...
    def get_channel_state(self, channel: int, timeout: Optional[float] = None) -> Optional[bool]:
        ...
        with self._lock:
            ...
            states = self._query_locked(serial_obj, timeout)
        if states is None:
            logger.warning(f"[RELAY] 第{channel}路状态查询失败: 未收到有效返回")
            return None
        if channel not in states:
            logger.warning(f"[RELAY] 第{channel}路状态未出现在返回数据中({format_states(states)})")
            return None
        state = states[channel]
        logger.info(f"[RELAY] 第{channel}路当前状态: {'ON' if state else 'OFF'}")
        return state
    ...
    def detect_channel_count(self, timeout: Optional[float] = None) -> Optional[int]:
        ...
        - 用 8 路默认配置去查询 4 路板时, ``query_status()`` 会告警缺少通道 [5..8], 属预期行为,
          这时请用 ``LCUSRelay(..., channel_count=4)`` 或 CLI 的 ``--channels 4``。
        """
        with self._lock:
            serial_obj = self._require_serial()
        ...
        if not states:
            logger.warning("[RELAY] 路数识别失败: 未收到有效返回")
            return None
        detected = max(states)
        logger.info(f"[RELAY] 检测到 {detected} 路继电器({format_states(states)})")
        return detected
    ...
    def _query_locked(
        ...
        serial_obj.flush()
        logger.debug(f"[RELAY] 发送状态查询 {_hex(bytes((QUERY_BYTE,)))}")
        ...
        if missing and warn_missing:
            logger.warning(f"[RELAY] FF 状态查询缺少通道: {missing}")
        logger.debug(f"[RELAY] FF 原始返回: {buffer!r}")
        return states
